...
/**
 * Defines the query for a specific partition. Operators define the set of result uids and predicates to receive and are
 * all mandatory. If no Get operators are given, predicates for all Has operators are retrieved.
 *
 * Operators are evaluated as AND.
 *
 * @param resultName
 *   result name in the JSON query
 * @param operators
 *   set of operators
 */
case class PartitionQuery(resultName: String, operators: Set[Operator]) {
  ...
  val predicateVals: Map[String, String] =
    hasPredicates.flatten.toSeq.sorted.zipWithIndex.map { case (predicate, idx) =>
      predicate -> s"pred${idx + 1}"
    }.toMap
  ...
  val (hasProperties, hasEdges) =
    operators
      .filter(_.isInstanceOf[Has])
      .map { case Has(properties, edges) => (properties, edges) }
      .fold((Set.empty[String], Set.empty[String])) { case ((leftP, leftE), (rightP, rightE)) =>
        (leftP ++ rightP, leftE ++ rightE)
      }
  ...
  /**
   * Provides the GraphQl query for the given chunk, or if no chunk is given the query for the entire result set.
   *
   * @param chunk
   *   optional chunk
   * @return
   *   result set or chunk of it
   */
  ...
  def getFilter(operator: Operator, predicateName: String, filterMode: String, andMode: Boolean): String =
    operator match {
  ...
  def getPredicateQueries(chunk: Option[Chunk]): Map[String, String] = {
    ...
    hasPredicates.flatten
      .map(pred =>
        predicateVals(pred) -> s"""${predicateVals(pred)} as var(func: has(<$pred>${lang(pred)})${getChunkString(
            chunk
          )})${getValueFilter(pred, "uids")}"""
      )
      .toMap
  }
  ...
  val predicatePaths: Seq[String] = {
    def lang(pred: String): String = if (langPredicates.contains(pred)) "@*" else ""
    (getProperties.map(pred => pred -> s"<$pred>${lang(pred)}") ++ getEdges.map(edge =>
      edge -> s"<$edge> { uid }"
    )).toSeq
      .sortBy(_._1)
      .map { case (pred, path) =>
