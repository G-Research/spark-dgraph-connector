...
/**
 * A sequence of uids is considered sparse if there are no more than denseGapRepetitionThreshold consecutive dense gaps.
 *
 * @param denseGapThreshold
 *   gaps below this threshold are considered dense
 * @param denseGapRepetitionThreshold
 *   observe consecutive dense gaps to consider uids dense
 */
case class ThresholdSparseDetector(denseGapThreshold: Int, denseGapRepetitionThreshold: Int) extends SparseDetector {
  if (denseGapRepetitionThreshold < 1)
    throw new IllegalArgumentException(s"The denseGapRepetitionThreshold must be larger than zero: $denseGapThreshold")
    ...
    if (uids.length <= denseGapRepetitionThreshold) {
      ...
      val repetitions = getSparseGaps(uids)
        .foldLeft(0)((count, gap) =>
          ...
          else if (gap.compareTo(denseGapThresholdUnsigned) < 0) count + 1
          else 0
        )
