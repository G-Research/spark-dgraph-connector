...
case class JsonGraphQlUidDetector(executor: JsonGraphQlExecutor) extends UidDetector with JsonGraphQlResultParser with Logging {
  /**
   * Query the partition for consecutive uids starting at the given uid.
   *
   * @param partition partition to query
   * @param uid       first uids to retrieve, must be larger than 0x0
