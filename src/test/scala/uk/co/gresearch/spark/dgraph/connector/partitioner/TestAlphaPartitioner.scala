  ...
  describe("AlphaPartitioner") {
    ...
    it("should partition with 1 partition per alpha") {
      ...
      assert(partitions.toSet === Set(
        ...
        Partition(Seq(Target("host3:9080"), Target("host2:9080")), Some(Set(Predicate("pred3", "type3"), Predicate("pred4", "type4"))), None, None),

        Partition(Seq(Target("host4:9080"), Target("host5:9080")), Some(Set(Predicate("pred5", "type5"))), None, None),

        Partition(Seq(Target("host6:9080")), Some(Set(Predicate("pred7", "type7"), Predicate("pred6", "type6"))), None, None)
      ))
    }
    ...
    Seq(2, 3, 7).foreach(partsPerAlpha =>
      it(s"should partition with $partsPerAlpha partitions per alpha") {
        ...
        assert(partitions.toSet === Set(
          ...
          Partition(Seq(Target("host3:9080"), Target("host2:9080")), Some(Set(Predicate("pred4", "type4"))), None, None),

          Partition(Seq(Target("host4:9080"), Target("host5:9080")), Some(Set(Predicate("pred5", "type5"))), None, None),

          Partition(Seq(Target("host6:9080")), Some(Set(Predicate("pred6", "type6"))), None, None),
          Partition(Seq(Target("host6:9080")), Some(Set(Predicate("pred7", "type7"))), None, None)
        ))
      }
    )
