...
```sbt
libraryDependencies += "uk.co.gresearch.spark" %% "spark-dgraph-connector" % "0.9.0-3.5"
```
...
```shell script
spark-shell --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.9.0-3.5
```
...
```shell script
pyspark --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.9.0-3.5
```

Run your Python scripts that use PySpark (pyspark ≥3.0.0) and the Spark Dgraph Connector (version ≥0.5.0) via `spark-submit`:

```shell script
spark-submit --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.9.0-3.5 [script.py]
```
