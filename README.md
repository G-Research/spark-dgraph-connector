...
The Spark Dgraph Connector is available for Spark 3.0, Spark 3.1 and Spark 3.2, all with Scala 2.12.
Use Maven artifact ID `spark-dgraph-connector_2.12`. The Spark version is part of the package version,
i.e. 0.8.0-3.0, 0.8.0-3.1 and 0.8.0-3.2, respectively.
...
Add this line to your `build.sbt` file to use the latest version for Spark 3.3:

```sbt
libraryDependencies += "uk.co.gresearch.spark" %% "spark-dgraph-connector" % "0.8.0-3.3"
```
...
```shell script
spark-shell --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.8.0-3.3
```
...
```shell script
pyspark --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.8.0-3.3
```

Run your Python scripts that use PySpark (pyspark ≥3.0.0) and the Spark Dgraph Connector (version ≥0.5.0) via `spark-submit`:

```shell script
spark-submit --packages uk.co.gresearch.spark:spark-dgraph-connector_2.12:0.8.0-3.3 [script.py]
```
