...
version=${DGRAPH_TEST_CLUSTER_VERSION:-v21.12.0}

if [[ $version > "v" && $version < "v21.03.0" ]]
then
...
docker run --rm -it -p 8080:8080 -p 9080:9080 -p 8000:8000 -p 6080:6080 -v "$(cd "$(dirname "$0")"; pwd)/dgraph-instance:/dgraph" dgraph/dgraph:${version} /bin/bash -c "dgraph zero & dgraph alpha $whitelist"
