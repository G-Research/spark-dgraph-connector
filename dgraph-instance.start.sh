...
version=${DGRAPH_TEST_CLUSTER_VERSION:-24.1.2}
...
docker run --rm -it -p 8080:8080 -p 9080:9080 -p 8000:8000 -p 6080:6080 -v "$(cd "$(dirname "$0")"; pwd)/dgraph-instance:/dgraph" dgraph/dgraph:v${version} /bin/bash -c "dgraph zero & dgraph alpha $whitelist"
